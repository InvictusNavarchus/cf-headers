...
import { cacheControl, immutableAssetCacheControl, noCacheControl } from "../src/helpers/cache-control.js";
import { csp, strictCsp } from "../src/helpers/csp.js";
import { lockedDownPermissionsPolicy, permissionsPolicy } from "../src/helpers/permissions-policy.js";
import {
  corsPreset,
...
describe("cacheControl", () => {
  it("builds a directive list in a stable order", () => {
    expect(cacheControl({ public: true, maxAge: 3600, immutable: true })).toBe("public, max-age=3600, immutable");
  });
  ...
  it("rejects public+private together", () => {
    expect(() => cacheControl({ public: true, private: true })).toThrow(/mutually exclusive/);
  });

  it("rejects noStore combined with maxAge", () => {
    expect(() => cacheControl({ noStore: true, maxAge: 60 })).toThrow(/noStore/);
  });
...
describe("csp", () => {
  it("joins directives with '; ' and sources with a space", () => {
    expect(csp({ defaultSrc: ["'self'"], scriptSrc: ["'self'", "https://cdn.example.com"] })).toBe(
  ...
  it("strictCsp() produces a locked-down baseline", () => {
    expect(strictCsp()).toBe("default-src 'self'; object-src 'none'; frame-ancestors 'none'; base-uri 'self'");
  });
...
describe("permissionsPolicy", () => {
  ...
  it("renders 'self' unquoted and other origins quoted", () => {
    expect(permissionsPolicy({ geolocation: ["self", "https://example.com"] })).toBe(
  ...
  it("supports arbitrary feature names via `extra`", () => {
    expect(permissionsPolicy({ extra: { "attribution-reporting": [] } })).toBe("attribution-reporting=()");
  });
...
describe("presets", () => {
  ...
  it("immutableAssetsPreset uses the immutable cache-control value", () => {
    ...
    expect(p.headers["Cache-Control"]).toBe("public, max-age=31536000, immutable");
  });
